...
@section('content')
        ...
        @else
            <div class="row">
                @foreach ($dataFikes as $item)
                                    ...
                                    <div class="col-9">
                                        <div class="row align-items-center">
                                            ...
                                            <div class="col-8">
                                                <div class="row">
                                                    <div class="col-12">
                                                        <p class="card-text mb-0">{{ $item->nama_wakil }} </p>
                                                        <p class="card-text mb-2"><button class="btn btn-sm btn-outline-primary" style="border-radius: 45px">Biodata</button> </p>
                                                    </div>
                                                </div>
                                            </div>
                    ...
                    <form action="{{ route('storeVoting3', $item->id) }}" id="voting_{{ $item->id }}" method="post">
                        @method('POST')
                        @csrf
                    </form>
                @endforeach
    ...
    <script>
        ...
        function showConfirmation(id) {
            $.ajax({
                url: '{{ route("searchPresmaFikesId",":id") }}'.replace(':id',id),
                data: {
                    id:id
                },
                type : 'GET',
